...
#import sklearn  # For scikit-learn version
...
def accident_severity_prediction_tab(df_merged_with_casualty_info):
    ...
    try:
        if df_merged_with_casualty_info is not None:
            ...
            st.write(f"Joblib Version: {joblib.__version__}")  #check joblib version
            st.write(f"Number of features in X_train: {X_train.shape[1]}") #shape of training data.
            st.write(f"Data types in X_train: {X_train.dtypes}")
            # -----------------------------
            ...
            if rf_model is not None:
                try:
                    ...
                    st.error(f"Error during Random Forest model evaluation: {e}")
            else:
                st.warning("Random Forest model not loaded, skipping evaluation.")
            ...
            if nn_model is not None:
                try:
                    ...
                    st.error(f"Error during Neural Network model evaluation: {e}")
            else:
                st.warning("Neural Network model not loaded, skipping evaluation.")
        else:
...
if df_casualties is not None:
    try:
        ...
        df_casualty_aggregation = pd.merge(df_max_casualty_severity, df_serious_casualties, on='accident_index', how='outer').fillna(0)
...
df_merged_with_casualty_info = None
...
if df_collision is not None and df_merged_with_casualty_info is not None:
    try:
        ...
        df_merged_with_casualty_info['date'] = pd.to_datetime(df_merged_with_casualty_info['date'], errors='coerce')
        df_merged_with_casualty_info['month'] = df_merged_with_casualty_info['date'].dt.month
        ...
        available_months = sorted(df_merged_with_casualty_info['month'].dropna().unique())
...
if df_merged_with_casualty_info is not None:
    try:
        ...
        df_filtered = df_merged_with_casualty_info[
